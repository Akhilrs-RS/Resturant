...
for filename, item_key in uploads:
    ...
    try:
        req = urllib.request.Request(f"{API_BASE_URL}/catalog/prices/{item_key}", data=payload, method='PUT')
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"Failed: {item_key} - {response.status}")
    except urllib.error.URLError as e:
        print(f"Error: {item_key} - {e}")
