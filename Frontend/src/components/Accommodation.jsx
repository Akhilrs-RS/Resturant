...
export default function Accommodation({ handleOpenBooking, handleScrollTo, setCurrentPage }) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setRoomsList(prev => prev.map(room => {
            let dbKey = '';
            if (room.id === 'ocean-premium') dbKey = 'suite_ocean';
            else if (room.id === 'canopy-family') dbKey = 'suite_deluxe';
            else if (room.id === 'garden-deluxe') dbKey = 'suite_deluxe';
            else if (room.id === 'private-pool') dbKey = 'suite_ocean';
            ...
            return match ? { 
              ...
              image: match.imageUrl || room.image 
            } : room;
          }));
        }
      } catch (err) {
        console.error("Failed to fetch catalog prices in Accommodation:", err);
      }
    };
    fetchPrices();
  }, []);
  ...
  const filteredRooms = activeCategory === 'All'
    ? roomsList
    : roomsList.filter(room => room.category.toLowerCase() === activeCategory.toLowerCase());
