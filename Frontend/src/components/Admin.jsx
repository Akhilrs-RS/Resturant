...
import { Shield, BookOpen, Utensils, RefreshCw, Trash2, Check, Send, AlertTriangle, Wine, Image } from 'lucide-react';
...
export default function Admin({ handleScrollTo, setCurrentPage }) {
  ...
  const [editDietaryType, setEditDietaryType] = useState('');
      ...
      <div className="bg-zinc-50 min-h-screen text-black font-sans flex items-center justify-center p-6 select-none">
        <div className="w-full max-w-md bg-white border border-gray-200 rounded-3xl p-8 shadow-xl relative">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <h2 className="font-times text-2xl font-light text-black tracking-wide">
        ...
        <aside className="w-full lg:w-64 bg-white border-r border-gray-200 p-6 space-y-2">
          ...
          <div className="pt-6">
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 text-xs font-semibold uppercase tracking-wider rounded-xl transition-all duration-300 ${
                activeTab === 'homeimages' 
                  ? 'bg-black text-white' 
                  : 'text-gray-600 hover:bg-gray-100 hover:text-black'
              }`}
            >
              <Image className="w-4 h-4" /> Manage Home Images
            </button>
        ...
        <main className="flex-1 p-6 md:p-8 space-y-6 overflow-x-hidden text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
              ...
              <p className="font-times text-3xl font-light mt-2 text-black">{suiteBookings.length}</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Table Bookings</span>
              <p className="font-times text-3xl font-light mt-2 text-black">{tableReservations.length}</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Lounge bookings</span>
              <p className="font-times text-3xl font-light mt-2 text-black">{loungeReservations.length}</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Inquiries Received</span>
              <p className="font-times text-3xl font-light mt-2 text-black">{inquiries.length}</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="px-6 py-5 border-b border-gray-200 flex items-center justify-between">
              <h2 className="font-times text-base font-light text-black capitalize">
                ...
                {activeTab === 'homeimages' && 'Manage Home Page Dynamic Images'}
              </h2>
            </div>
            ...
            <div className="overflow-x-auto w-full">
              ...
              {activeTab === 'homeimages' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
                  {catalogPrices.filter(item => item.category === 'Home_Images').map((room) => {
                    const isEditingThis = editingKey === room.itemKey;
                    return (
                      <div key={room.id} className="bg-white border border-gray-200 rounded-3xl p-6 flex flex-col justify-between space-y-6">
                        ...
                        <div className="space-y-4">
                          <div>
                            <label className="text-[10px] text-gray-500 block font-semibold uppercase tracking-wider mb-2">Section Name</label>
                            {isEditingThis ? (
                              <input 
                                type="text"
                                value={editRoomTitle}
                          ...
                          <div>
                            <label className="text-[10px] text-gray-500 block font-semibold uppercase tracking-wider mb-2">Internal Key</label>
                            <span className="font-mono text-xs text-gray-400 block py-2">{room.itemKey}</span>
                          </div>

                          {isEditingThis && (
                            <div>
                              <label className="text-[10px] text-gray-500 block font-semibold uppercase tracking-wider mb-2">Upload Section Image</label>
                              <input
                                type="file"
                                accept="image/*"
                                onChange={(e) => {
